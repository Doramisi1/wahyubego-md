let handler = (m, { conn, usedPrefix, command, text }) => {
  conn.tembak = conn.tembak || { musuh: [], tembak: [] }
   if(/kiri/i.test(text)) {
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.sendButton(m.chat, pos, "©yoga Prasetyo", "Tembak", `${usedPrefix}${command} tembak`)
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.sendButton(m.chat, pos, "©yoga Prasetyo", "Tembak", `${usedPrefix}${command} tembak`)
    return conn.send2Button(m.chat, pos, "©yoga prasetyo", "←", `${usedPrefix}${command} kiri`, "→", `${usedPrefix}${command} kanan`)
