...
handler.before = async function (m) {
    ...
    if (!(id in this.siapakahaku)) return m.reply('Soal itu telah berakhir lu telat')
    if (m.quoted.id == this.siapakahaku[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*Benar!* +${this.siapakahaku[id][2]} XP`, '© BEDUL BOT', 'Siapakah Aku', '.siapaaku')
